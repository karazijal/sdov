...
class LocalAffinity(nn.Module):
    ...
    def forward(self, x):
        ...
        s = self.kernel.shape[0]
        x_affs = torch.empty(B*K, len(self.dilations)*s, H, W, device=x.device)
        for i,d in enumerate(self.dilations):
            x_pad = F.pad(x, [d]*4, mode='replicate')
            x_affs[:, i*s:(i+1)*s] = F.conv2d(x_pad, self.kernel, dilation=d)
            
        return x_affs.view(B,K,-1,H,W)
...
class PAMR(nn.Module):
    ...
    def __init__(self, num_iter=10, dilations=[1, 2, 4, 8, 12, 16]):
    ...
    @property
    def device(self):
        return self.aff_std.kernel.device
    
    def forward(self, x, mask):
        out_device = mask.device
        mask = F.interpolate(mask, size=x.size()[-2:], mode="bilinear", align_corners=True).to(self.device)
        x = x.to(self.device)
        # x: [BxKxHxW]
        # mask: [BxCxHxW]
        B,K,H,W = x.size()
